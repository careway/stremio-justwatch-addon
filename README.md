...
Once signed in, `/configure` becomes a small app with a menu: **Manifest** (the install link plus one tab per selection, with the usual country/provider picker), **Configuración** (cover-rating provider and key, randomize, hide country, TMDb key, regenerate the link, delete the account, sign out) and **Suscripción** (current plan, usage, and the plan comparison). Those settings are per account and apply to every selection; the poster key is stored server-side and never sent back to the page.

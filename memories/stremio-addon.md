...
- `src/index.js` — HTTP server, router, logger, `getLanguageFromRequest()`; exports `handler` for Vercel, `http.createServer` only under `require.main === module`; file logging skipped when `NODE_ENV=production`
- `src/justwatch.js` — JustWatch GraphQL (`GetPopularTitles`), two-layer cache (L1 in-memory + L2 Redis, 12h TTL); Redis configured via `REDIS_URL` (takes precedence) or `REDIS_HOST`/`REDIS_PORT`
...
- `api/index.js` — Vercel serverless entry point (`module.exports = require('../src/index')`)
- `vercel.json` — rewrites all routes to `/api`
- `.env.example` — documents `PORT`, `REDIS_HOST`/`REDIS_PORT`, `REDIS_URL`
